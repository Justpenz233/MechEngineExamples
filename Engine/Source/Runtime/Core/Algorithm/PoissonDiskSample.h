...
{
  // Poisson Disk Sampling with set radius, by default all dimensions in [0, 1]
  template <int Dimension>
  std::vector<Eigen::VectorXd> PoissonDiskSampling(double Radius)
  {
    int SampleCount = std::pow(1. / Radius, Dimension);
    LOG_INFO("Poisson Disk Sampling: Trying to sample {} with radius {} from {}d space", SampleCount, Radius, Dimension);

    std::array<double, Dimension> kXMin;
    std::array<double, Dimension> kXMax;
    kXMin.fill(0.);
    kXMax.fill(1.);
    auto Result =  thinks::PoissonDiskSampling<double, Dimension>(
      Radius, kXMin, kXMax);
    LOG_INFO("Poisson Disk Sampling: Actually sampled {} with radius {} from {}d space", Result.size(), Radius, Dimension);
    vector<VectorXd> Samples;
    for(auto i : Result)
      Samples.emplace_back(Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(i.data(), Dimension));
    return Samples;
  }

  // Poisson Disk Sampling with eager Sample Count, by default all dimensions in [0, 1]
  // Notice the actual count will near but not equal to the eager count
  template <int Dimension>
  std::vector<Eigen::VectorXd> PoissonDiskSampling(int SampleCount)
  {
    double Radius = 1. / std::pow(SampleCount, 1. / Dimension);
    LOG_INFO("Poisson Disk Sampling: Trying to sample {} with radius {} from {}d space", SampleCount, Radius, Dimension);
    std::array<double, Dimension> kXMin;
    std::array<double, Dimension> kXMax;
    kXMin.fill(0.);
